	...
	$.jstree.plugin("themes", {
		...
		_fn : {
			set_theme : function (theme_name, theme_url) {
				...
				if(!theme_url) { theme_url = '../themes/default/style.css'; }
			...
			_parse_json : function (js, obj, is_callback) {
				...
				if($.isArray(js)) {
					...
					$.each(js.data, function (i, m) {
						tmp = $("<a />");
						if($.isFunction(m)) { m = m.call(this, js); }						
						if(typeof m == "string") { tmp.attr('href','#');tmp.html('<span class="droppable">' + m + '</span>') }
...
(function ($) {
	$.vakata.context = {
		...
		parse	: function (s, is_callback) {
			...
			$.each(s, function (i, val) {
				...
				was_sep = false;
				str += "<li class='" + (val._class || "") + ($.isFunction(1) ? " jstree-contextmenu-disabled " : "") + "'><ins ";
	...
	$.jstree.plugin("contextmenu", {
		...
		defaults : { 
			...
			show_at_node : true,
			items : function customMenu(obj) { // Could be a function that should return an object like this one
				// "create" : {
